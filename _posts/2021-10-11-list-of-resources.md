---
title: "Can you, like, REALLY explain me this?"
description: "My personal list of resources to look back to"
layout: post
toc: true
comments: true
hide: false
search_exclude: false
image: /images/list-of-resources/linked-tori.png
categories: [resources]
author: "<a href='https://www.linkedin.com/in/yash-jakhotiya/'>Yash Jakhotiya</a>"
---

There are times when you need to look back to something and reassure yourself that you _really_ _really_ understand it. This is a dynamic work-in-progress list of resources that has, potentially in a distant past, helped me do that.

# Linear Algebra :

1. [Change of basis](https://www.youtube.com/watch?v=P2LTAUO1TdA)

2. [Eigenvectors and Eigenvalues](https://www.youtube.com/watch?v=PFDu9oVAE-g)

# Statistical ML

## Probability, MLE, EM
1. [Multiple random variables probability review](https://courses.engr.illinois.edu/cs440/sp2011/slides/lecture15.pdf)

2. [Generative modeling, Latent variables, GMMs, MLE, EM](https://mahdi-roozbahani.github.io/CS46417641-fall2021/course/09-gaussian-mixture-note.pdf)

## Naive Bayes, Logistic - Sigmoid, Softmax
1. [MLE, Naive Bayes, Logistic](https://cocoxu.github.io/CS7650_fall2021/slides/lec2-ml.pdf)

2. [Logistic, Softmax, Cross-entropy, Logistic connection to a single neuron via sigmoid](www.youtube.com/watch?v=8CWyBNX6eDo)

# Neural Network Foundational

## Space Transformation
1. [Neural Networks, Manifolds, and Topology](http://colah.github.io/posts/2014-03-NN-Manifolds-Topology/)

## Backpropagation
1. [What is backpropagation really doing?](https://www.youtube.com/watch?v=Ilg3gGewQ5U)

2. [Backpropagation calculus](https://www.youtube.com/watch?v=tIeHLnjs5U8)

3. [Backprop with practical considerations](https://www.youtube.com/watch?v=yLYHDSv-288)